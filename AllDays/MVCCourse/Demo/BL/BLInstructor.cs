...
namespace Day2.BL
{
    public class BLInstructor
    {
        ...
        {
            var instructorList= instructorRepo.GetAll().Include(dept=>dept.Department).AsQueryable();
            var instructorVMList = mapper.Map<List<InstructorViewModel>>(instructorList.ToList());
